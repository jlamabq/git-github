...
resource "aws_dynamodb_table" "tf_backend_state_lock_table" {
  ...
  name             = "${var.project}-${var.region}_${var.dynamodb_lock_table_name}"
...
resource "aws_s3_bucket" "tf_backend_bucket" {
  # updated
  bucket = "${var.project}-${var.region}-${var.backend_bucket}"
  acl    = "private"
  versioning {
    enabled = true
  }
...
resource "aws_s3_bucket" "tf_backend_logs_bucket" {
  #updated
  bucket = "${var.project}-${var.region}-${var.backend_bucket}-logs"
  acl    = "log-delivery-write"


  logging {
      target_bucket = "${var.project}-${var.region}-server-access"
      target_prefix = "terraform_access-logs/"
  }
  ...
  tags = {
    Purpose            = "Logging bucket for ${var.project}-${var.region}-${var.backend_bucket}"
    ManagedByTerraform = "true"
    TerraformModule    = "terraform-aws-backend"
  }
